...
def kmeans(X, K, iters, id2user, data, SZ, deadline):
    ...
    for i in range(K):
#        ids = []
        ids = (CVal[i]).index(min(CVal[i]))
        vl.append(Center[i][ids])
        numb.append(len(Center[i]))
    tolnum = sum(numb)
    print vl
    print numb
    for i in range(K):
        if data[vl[i]][0] not in result:
            result[data[vl[i]][0]] = ''
        result[data[vl[i]][0]]  += str(data[vl[i]][1]) + ':' + str(int(numb[i]*1000/tolnum)) + ':' + deadline + '|'
    for i in result:
        result[i] = result[i][:-1]   
    return result

def call_represent(data, deadline, K=6):
#    print "data="+str(data)
    if (len(data) == 0):
        return ([], [])
    dist = [None]*len(data)
