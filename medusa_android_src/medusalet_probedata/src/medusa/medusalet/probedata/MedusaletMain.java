...
public class MedusaletMain extends MedusaletBase {
    ...
    {	
		...
		sqlFilename = this.getConfigParams("-file");
    ...
    public boolean run() 
	{
		...
		if (sqlTimeFrom != null || sqlTimeTo != null || sqlType != null || sqlFilename != null) {
			sqlstmt += " where";
			if (sqlTimeFrom != null) {
			...
			if (sqlFilename != null) {
				if (need_end) sqlstmt += " and ";
				String[] files = sqlFilename.split("\\|");
				sqlstmt += "(";
				for (int i=0; i<files.length; i++)
				{
					sqlstmt += "path like '%" + files[i] + "%'";
					sqlstmt += i == files.length - 1 ? "" : " or ";
				}
				sqlstmt += ")";
			}
		}
