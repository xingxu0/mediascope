/**
 * 'MedusaTransformMFCCAdapter'
 * 	
 * - original from the CoMIRVA project
 * - has a dependency on medusa.mobile.client.math.Matrix 
 * and MedusaTransformFFTAdapter
 * 
 * @modified : Nov. 4th, 2011
...
public class MedusaTransformMFCCAdapter {
	...
	private MedusaTransformFFTAdapter normalizedPowerFFT;
	private double scale;

	/**
	 * Creates a new MFCC object with default window size of 512 for the given
	 * sample rate. The overlap of the windows is fixed at 50 percent. The
	 * number of coefficients is set to 20 and the first coefficient is in use.
	 ...
	 */
	public MedusaTransformMFCCAdapter(float sampleRate)
			throws IllegalArgumentException {
		this(sampleRate, 512, 20, true, 20.0, 16000.0, 40, 256);
	}

	/**
	 * Creates a new MFCC object. 40 mel-filters are place in the range from 20
	 * to 16000 Hz.
	 * 
	 * @param sampleRate
	 *            float samples per second, must be greater than zero; not
	 *            whole-numbered values get rounded
	 * @param windowSize
	 *            int size of window; must be 2^n and at least 32
	 * @param numberCoefficients
	 *            int must be grate or equal to 1 and smaller than the number of
	 *            filters
	 * @param useFirstCoefficient
	 *            boolean indicates whether the first coefficient of the dct
	 *            process should be used in the mfcc feature vector or not
	 * @throws IllegalArgumentException
	 *             raised if method contract is violated
	 */
	public MedusaTransformMFCCAdapter(float sampleRate, int windowSize,
			int numberCoefficients, boolean useFirstCoefficient)
			throws IllegalArgumentException {
		this(sampleRate, windowSize, numberCoefficients, useFirstCoefficient,
				20.0, 16000.0, 40, 256);
	}
	 ...
	 *            process should be used in the mfcc feature vector or not
	 * @param minFreq
	 *            double start of the interval to place the mel-filters in
	 * @param maxFreq
	 *            double end of the interval to place the mel-filters in
	 * @param numberFilters
	...
	public MedusaTransformMFCCAdapter(float sampleRate, int windowSize,
			int numberCoefficients, boolean useFirstCoefficient,
			double minFreq, double maxFreq, int numberFilters, int overlapSize)
			throws IllegalArgumentException {
		...
		this.hopSize = windowSize - overlapSize; // hopSize
		...
		normalizedPowerFFT = new MedusaTransformFFTAdapter(
				MedusaTransformFFTAdapter.FFT_NORMALIZED_POWER, windowSize,
				MedusaTransformFFTAdapter.WND_HANNING);
		// normalizedPowerFFT = new FFT(FFT.FFT_NORMALIZED_POWER, windowSize,
		// FFT.WND_HAMMING);

		// compute rescale factor to rescale and normalize at once (default is
		// 96dB = 2^16)
		scale = (Math.pow(10, 96 / 20));
	}

	/**
	 * Returns the boundaries (start, center, end) of a given number of
	 * triangular mel filters at linear scale. Mel-filters are triangular
	 * filters on the linear scale with an integral (area) of 1. However they
	 * are placed equidistantly on the mel scale, which is non-linear rather
	 * logarithmic. The minimum linear frequency and the maximum linear
	 * frequency define the mel-scaled interval to equidistantly place the
	 * filters. Since mel-filters overlap, an array is used to efficiently store
	 * the boundaries of a filter. For example you can get the boundaries of the
	 * k-th filter by accessing the returned array as follows:
	 * 
	 * leftBoundary = boundaries[k-1]; center = boundaries[k]; rightBoundary =
	 * boundaries[k+1];
	 * 
	 * @param minFreq
	 *            double frequency used for the left boundary of the first
	 *            filter
	 * @param maxFreq
	 *            double frequency used for the right boundary of the last
	 *            filter
	 * @param numberFilters
	 *            int number of filters to place within the interval [minFreq,
	 *            maxFreq]
	 ...
	 * mel-filters. Each filter is represented by one row of this matrix. Thus
	 * all the filters can be applied at once by a simple matrix multiplication.
	 * 
	 ...
	 * (area) of 1. However they are placed equidistantly on the mel scale,
	 * which is non-linear rather logarithmic. Consequently there are lots of
	 * high, thin filters at start of the linear scale and rather few and flat
	 * filters at the end of the linear scale. Since the start-, center- and
	 * end-points of the triangular mel-filters on the linear scale are known,
	 * the weights are computed using linear interpolation.
	 * 
	 * @param filterBank
	 *            int the number of the mel-filter, used to extract the
	 *            boundaries of the filter from the array
	 * @param freq
	 *            double the frequency, at which the filter weight should be
	 *            returned
	 * @param boundaries
	 *            double[] an array containing all the boundaries
	...
	/**
	 * Generates the DCT matrix for the known number of filters (input vector)
	 * and for the known number of used coefficients (output vector). Therefore
	 * the DCT matrix has the dimensions (numberCoefficients x numberFilters).
	 ...
	 * splitting the given data into windows and processing each of these
	 * windows with processWindow().
	 * 
	 * @param input
	 *            double[] input data is an array of samples, must be a multiple
	 *            of the hop size, must not be a null value
	 * @return double[][] an array of arrays contains a double array of Sone
	...
	public double[][] process(double[] input) {
		// check for null
		if (input == null) {
			Log.e("MedusaTransformMFCCAdapter", "input data must not be a null value");		
			return null;
		}

		// check for correct array length
		if ((input.length % hopSize) != 0) {
			Log.e("MedusaTransformMFCCAdapter", "Input data must be multiple of hop size (windowSize/2).");
			return null;
		}
		...
		if (windowSize > hopSize) {
			mfcc = new double[(input.length / hopSize) - 1][numberCoefficients];
		} else if (windowSize == hopSize) {
			mfcc = new double[input.length / hopSize][numberCoefficients];
		}
		...
		if (windowSize == hopSize) {
			mfcc[mfcc.length - 1] = processWindow(input, input.length - hopSize);
		}
	...
	/**
	 * Transforms one window of MFCCs. The following steps are performed: <br>
	 * <br>
	...
	public double[] processWindow(double[] window, int start)
		...
		Boolean cont = false;
		// just copy to buffer and rescaled the input samples according to the
		// original matlab implementation to 96dB
		...
		if (cont) {
			...
			// use all coefficient up to the nyquist frequency
			// (ceil((fftSize+1)/2))
			Matrix x = new Matrix(buffer, windowSize);
			x = x.getMatrix(0, fftSize - 1, 0, 0); // fftSize-1 is the index of
			...
			// to db
			double log10 = 10 * (1 / Math.log(10)); // log for base 10 and scale
													// by factor 10
			...
			return new double[numberCoefficients];
		}
	}
