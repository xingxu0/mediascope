 ...
 *
 * - Implements WiFi only algorithm. 
...
public class MedusaAPControlWiFiOnly extends MedusaAPControlBase {
	double avgU;
	double miscU;

	MedusaAPControlWiFiOnly(HashMap<String, String> configmap) {
		super(configmap);
		// TODO Auto-generated constructor stub
		Log.i("APCtrl", "* create MedusaAPControlWiFiOnly() instance.");
	}
	...
	protected void printStats(APElement ap) {
		...
		Log.i("APCtrlWiFiOnly", String.format(
				"[STAT]\tMinEnergy\t%d\t%.6f\t%.6f", slotCnt, ureal, avgU));
	}
	...
	APElement selectAlgorithm() {
		...
		while (itr.hasNext() == true) {
			...
			if (cur.failcnt_ >= APElement.AP_FAIL_LIMIT)
				continue;
			if (cur.refresh_ >= APElement.AP_NO_SCAN_LIMIT
					&& (cur.networkmode_ == APElement.NETMODE_WLAN_INFRA || cur.networkmode_ == APElement.NETMODE_WLAN_ADHOC))
				continue;

			if ((cur.networkmode_ == APElement.NETMODE_WLAN_INFRA || cur.networkmode_ == APElement.NETMODE_WLAN_ADHOC)
					&& cur.rssi_ < ssThreshold) {
